...
void loop()
{
  ...
  if (currentMillis - lastUpdate >= updateInterval)
  {
    ...
    ambientTemperature = ambientSensor.temperature(RNOMINAL, RREF) * 1.8 + 32 + 7.0;
    temp1 = sensor1.temperature(RNOMINAL, RREF) * 1.8 + 32 - 25.0;
    temp2 = sensor2.temperature(RNOMINAL, RREF) * 1.8 + 32 + 14.0;
    temp3 = sensor3.temperature(RNOMINAL, RREF) * 1.8 + 32 - 3.0;
    temp4 = sensor4.temperature(RNOMINAL, RREF) * 1.8 + 32 - 1.0;
  ...
  if (buttonInputs.size() == 5)
  {
    ...
    if (buttonAverage > 10 && buttonAverage < 14)
    { // Increase button pressed
      Serial.println("Button increased");
      delta++;
      updateLCD();
    }
    else if (buttonAverage > 18 && buttonAverage < 23)
...
void sendLoraData()
{
  ...
  string combinedMessage = "2|"; // NEED TO CHANGE FOR EACH BOX
...
void changeVoltage(bool increase, float changeTarget)
{
  ...
  float percentVoltageReduction = 0.80; // Variac is harder to turn at higher voltage. Reduce expected voltage chnage if at higher voltage.
  ...
  else
  {
    ...
    if (currentVoltage > 96)
    {
      Serial.println("Voltage above 95.");
      currentVoltage -= averageVoltageChange * percentVoltageReduction;
    }
    else
    {
