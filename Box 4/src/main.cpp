...
void loop()
{
  ...
  if (currentMillis - lastUpdate >= updateInterval)
  {
    ...
    ambientTemperature = ambientSensor.temperature(RNOMINAL, RREF) * 1.8 + 32 + 4.5;
    temp1 = sensor1.temperature(RNOMINAL, RREF) * 1.8 + 32 + 19.0;
    temp2 = sensor2.temperature(RNOMINAL, RREF) * 1.8 + 32 - 3.0;
    temp3 = sensor3.temperature(RNOMINAL, RREF) * 1.8 + 32 + 11.0;
    temp4 = sensor4.temperature(RNOMINAL, RREF) * 1.8 + 32 + 5.0;
...
void changeVoltage(bool increase, float changeTarget)
{
  ...
  float percentVoltageReduction = 0.66; // Variac is harder to turn at higher voltage. Reduce expected voltage chnage if at higher voltage.
