...
const unsigned long loraUpdateInterval = 15000; // Send every 15 minutes

float ambientTemperature, temp1, temp2, temp3, temp4, average, maxSensor, minSensor; // Store sensor values
float delta = 10.0;
...
void loop()
{
  ...
  // if (buttonState > 25 && buttonState < 40 ) { // Increase button pressed
  //   delta++;
  //   updateLCD();
  // }
  // else if (buttonState < 21) { // Decrease button presed
  //   delta--;
  //   updateLCD();
  // }
...
void sendLoraData()
{
  std::vector<float> dataToSend = {average, ambientTemperature, delta, currentVoltage, temp1, temp2, temp3, temp4};
...
int maintainTemperature()
{
  ...
  float averageMinusTarget = currentAverage - (delta + ambientTemperature);
  Serial.print("Target: ");
  Serial.println(target);
...
void resetVoltageToZero()
{
  ...
  Serial.print("Turns needed to reset to 0: ");
  Serial.println(turnsNeeded);
...
void changeVoltage(bool increase, float changeTarget)
{
  ...
  float percentVoltageReduction = 0.60; // Variac is harder to turn at higher voltage. Reduce expected voltage chnage if at higher voltage.
